...
function provisioning_get_hf_repo_models() {
    ...
    if /opt/environments/python/comfyui/bin/huggingface-cli download "$repo" \
        --cache-dir "$WORKSPACE/.cache" \
        --local-dir "$temp_dir" 2>/dev/null; then
        ...
        printf "WARNING: Failed to download from %s - skipping\n\n" "$repo"
    fi
